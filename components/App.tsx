...
import QuickInspectPopover from './QuickInspectPopover';
...
export const App = (): JSX.Element => {
  ...
  return (
    <div className="flex h-screen bg-[#24272E] text-[rgba(255,255,255,0.87)] font-sans">
      {/* Left Controls Panel */}
      <div className={\`neumorphic-panel flex flex-col \${isLeftPanelCollapsed ? COLLAPSED_WIDTH_CLASS : EXPANDED_WIDTH_LEFT_CLASS} transition-all duration-300 ease-in-out border-r border-[rgba(255,255,255,0.08)]\`}>
        {isLeftPanelCollapsed ? (
          <div className="flex flex-col items-center justify-center h-full py-4">
            <button onClick={() => setIsLeftPanelCollapsed(false)} className="neumorphic-button p-2.5 mb-4 hover:bg-sky-600/30" title="Expand Controls Panel">
              <Icons.ChevronDoubleRightIcon className="w-5 h-5 text-gray-300 hover:text-sky-300" />
            </button>
          ...
          <>
            ...
            <div className="flex-grow overflow-y-auto min-h-0">
             ...
             <div className="p-3 border-t border-[rgba(255,255,255,0.08)] space-y-2 flex-shrink-0">
                <h3 className="text-xs font-semibold text-[rgba(255,255,255,0.6)] mb-1.5 tracking-wider uppercase">History</h3>
                <div className="flex space-x-2">
                    <button
                        onClick={handleUndoWrapper}
                        disabled={!canUndo}
                        className={\`flex-1 flex items-center justify-center px-3 py-1.5 text-sm font-medium rounded-md neumorphic-button hover:text-sky-300 focus:ring-2 focus:ring-sky-400 focus:ring-offset-2 focus:ring-offset-[#2D3039] \${!canUndo ? 'disabled-look' : ''}\`}
                    ...
                    <button
                        ...
                        disabled={!canRedo}
                        className={\`flex-1 flex items-center justify-center px-3 py-1.5 text-sm font-medium rounded-md neumorphic-button hover:text-sky-300 focus:ring-2 focus:ring-sky-400 focus:ring-offset-2 focus:ring-offset-[#2D3039] \${!canRedo ? 'disabled-look' : ''}\`}
                        title="Redo (Ctrl/Cmd+Y or Ctrl/Cmd+Shift+Z)"
                    >
                        <Icons.ArrowUturnRightIcon className="w-4 h-4 mr-1.5" /> Redo
                    </button>
                </div>
                <h3 className="text-xs font-semibold text-[rgba(255,255,255,0.6)] mb-1.5 pt-2 tracking-wider uppercase">Clipboard</h3>
                <div className="flex space-x-2">
                    <button
                        onClick={handleCopyToClipboardWrapper}
                        disabled={selectedNodeIds.length === 0}
                        className={\`flex-1 flex items-center justify-center px-3 py-1.5 text-sm font-medium rounded-md neumorphic-button hover:text-sky-300 focus:ring-2 focus:ring-sky-400 focus:ring-offset-2 focus:ring-offset-[#2D3039] \${selectedNodeIds.length === 0 ? 'disabled-look' : ''}\`}
                    ...
                    <button
                        ...
                        disabled={!canPaste}
                        className={\`flex-1 flex items-center justify-center px-3 py-1.5 text-sm font-medium rounded-md neumorphic-button hover:text-sky-300 focus:ring-2 focus:ring-sky-400 focus:ring-offset-2 focus:ring-offset-[#2D3039] \${!canPaste ? 'disabled-look' : ''}\`}
            ...
            <div className="p-2 border-t border-[rgba(255,255,255,0.08)] flex-shrink-0">
              <button onClick={() => setIsLeftPanelCollapsed(true)} className="w-full neumorphic-button p-2 flex items-center justify-center hover:bg-gray-700/50" title="Collapse Controls Panel">
                <Icons.ChevronDoubleLeftIcon className="w-5 h-5 text-gray-300 hover:text-sky-300" />
                <span className="ml-2 text-xs text-gray-300 hover:text-sky-300">Collapse</span>
              </button>
            </div>
          </>
        )}
      ...
      <div className="flex-1 flex flex-col max-h-screen overflow-hidden"> {/* Added overflow-hidden */}
        <div className="p-4 flex flex-col space-y-3 flex-grow min-h-0">
             <div className="flex space-x-3 items-center mb-0 pb-0 flex-shrink-0">
                <h1 className="text-2xl font-bold text-[rgba(255,255,255,0.9)]" style={{textShadow: '0 1px 3px rgba(0,0,0,0.3)'}}>Logical Construction Plan</h1>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-1 items-start flex-shrink-0 p-3 rounded-lg neumorphic-panel">
                <div className="flex flex-col space-y-1">
                    <label htmlFor="eventNameInput" className="text-xs text-[rgba(255,255,255,0.6)]">Event Name:</label>
                    <input
                        id="eventNameInput" type="text" value={eventNameInput} onChange={e => setEventNameInput(e.target.value)}
                        placeholder="e.g., buttonClick"
                        className="p-1.5 text-sm bg-[#252830] border border-[rgba(255,255,255,0.1)] rounded-md focus:ring-1 focus:ring-[#a855f7] focus:border-[#a855f7] placeholder-gray-500"
                    />
                </div>
                <div className="flex flex-col space-y-1">
                     <label htmlFor="eventPayloadInput" className="text-xs text-[rgba(255,255,255,0.6)]">Event Payload (JSON):</label>
                    <textarea
                        ...
                        className="p-1.5 text-sm bg-[#252830] border border-[rgba(255,255,255,0.1)] rounded-md focus:ring-1 focus:ring-[#a855f7] focus:border-[#a855f7] h-9 resize-none placeholder-gray-500"
                    />
                </div>
                <button
                    onClick={() => handleDispatchEventWrapper()}
                    disabled={isExecuting && !executionMeta?.pausedNodeId}
                    className={\`self-end w-full h-9 flex items-center justify-center px-3 py-1.5 text-sm font-medium rounded-md primary-action-button focus:ring-2 focus:ring-offset-2 focus:ring-offset-[#2D3039] focus:ring-[#a855f7] \${(isExecuting && !executionMeta?.pausedNodeId) ? 'disabled-look' : ''}\`}
                >
      ...
      <div className={\`neumorphic-panel flex flex-col \${isRightPanelCollapsed ? COLLAPSED_WIDTH_CLASS : EXPANDED_WIDTH_RIGHT_CLASS} transition-all duration-300 ease-in-out border-l border-[rgba(255,255,255,0.08)]\`}>
        {isRightPanelCollapsed ? (
            <div className="flex flex-col items-center justify-center h-full py-4">
                <button onClick={() => setIsRightPanelCollapsed(false)} className="neumorphic-button p-2.5 mb-4 hover:bg-sky-600/30" title="Expand Inspector Panel">
                    <Icons.ChevronDoubleLeftIcon className="w-5 h-5 text-gray-300 hover:text-sky-300" />
                </button>
            ...
            <>
                <div className="flex-grow overflow-y-auto min-h-0">
                    {selectedNodeForInspector ? (
                        ...
                        <div className="p-3 space-y-2 flex-shrink-0"> {/* This wrapper is for when NO node is selected */}
                            <h3 className="text-xs font-semibold text-[rgba(255,255,255,0.6)] mb-1.5 tracking-wider uppercase">Graph Actions</h3>
                            <div className="space-y-2">
                                <button
                                    onClick={handleExecuteSelectedOutputWrapper}
                                    disabled={selectedNodeIds.length !== 1 || isExecuting}
                                    className={\`w-full flex items-center justify-center px-3 py-2 text-sm font-medium rounded-md turquoise-action-button \${(selectedNodeIds.length !== 1 || isExecuting) ? 'disabled-look' : ''}\`}
                                ...
                                <button
                                    ...
                                    className="w-full flex items-center justify-center px-3 py-2 text-sm font-medium rounded-md neumorphic-button hover:text-red-400 focus:ring-2 focus:ring-red-500 focus:ring-offset-2 focus:ring-offset-[#2D3039]"
                                ...
                                <button
                                    ...
                                    className="w-full flex items-center justify-center px-3 py-2 text-sm font-medium rounded-md neumorphic-button hover:text-teal-400 focus:ring-2 focus:ring-teal-500 focus:ring-offset-2 focus:ring-offset-[#2D3039]"
                                ...
                                <div className="flex space-x-2">
                                    <button
                                        ...
                                        className={\`flex-1 flex items-center justify-center px-3 py-2 text-sm font-medium rounded-md neumorphic-button hover:text-cyan-400 focus:ring-2 focus:ring-cyan-500 focus:ring-offset-2 focus:ring-offset-[#2D3039] \${selectedNodeIds.length < 2 ? 'disabled-look' : ''}\`}
                                ...
                                <div className="flex items-center justify-between pt-2">
                                    <span className="text-xs font-medium text-[rgba(255,255,255,0.7)]">Sequence Mode</span>
                                ...
                                <div className="flex items-center justify-between pt-1">
                                    <span className="text-xs font-medium text-[rgba(255,255,255,0.7)]">Focus Mode</span>
                            ...
                            <div className="pt-2">
                                <h3 className="text-xs font-semibold text-[rgba(255,255,255,0.6)] mb-1.5 tracking-wider uppercase">Persistence</h3>
                                <div className="space-y-2">
                                <button
                                    onClick={handleSaveGraph}
                                    className="w-full flex items-center justify-center px-3 py-2 text-sm font-medium rounded-md primary-action-button focus:ring-2 focus:ring-offset-2 focus:ring-offset-[#2D3039] focus:ring-[#a855f7]"
                                >
                                ...
                                <label
                                    ...
                                    className="w-full flex items-center justify-center px-3 py-2 text-sm font-medium rounded-md primary-action-button cursor-pointer focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-offset-[#2D3039] focus-within:ring-[#a855f7]"
                                >
                             ...
                             <div className="p-4 text-[rgba(255,255,255,0.5)] italic text-center mt-5">
                                Select a node to inspect its properties.
                            </div>
                        </div>
                    )}
                </div>
                <div className="h-[250px] flex-shrink-0 border-t border-[rgba(255,255,255,0.08)]">
                    <LogView logs={executionLogs} />
                </div>
                <div className="p-2 border-t border-[rgba(255,255,255,0.08)] flex-shrink-0">
                    <button onClick={() => setIsRightPanelCollapsed(true)} className="w-full neumorphic-button p-2 flex items-center justify-center hover:bg-gray-700/50" title="Collapse Inspector Panel">
                        <Icons.ChevronDoubleRightIcon className="w-5 h-5 text-gray-300 hover:text-sky-300" />
                         <span className="ml-2 text-xs text-gray-300 hover:text-sky-300">Collapse</span>
                    </button>
                </div>
