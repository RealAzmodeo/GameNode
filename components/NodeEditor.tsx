...
> {
  ...
  // Quick Inspect Popover data is passed directly if needed by NodeEditor itself, or handlers are passed to NodeComponent
  quickInspectPopoverData: QuickInspectData | null; // Keep this if NodeEditor needs direct access
}
...
const NodeEditor = forwardRef<NodeEditorRef, NodeEditorProps>(({
  ...
  executionMeta,
  // Quick Inspect Props from useQuickInspect hook, passed down from App
  ...
  const getPortPositionAndType = useCallback((nodeId: NodeId, portId: string, nodePortDirection: 'input' | 'output'): { x: number; y: number, portType: PortTypeEnum, portCategory: LogicalCategoryEnum, port: Port } | null => {
    ...
    return {
      ...
      port: port, // Return the full port object
    };
  }, [nodes]);
    ...
    if (connectingState.active && connectingState.sourceNodeId && connectingState.sourcePortId) {
      ...
      if (hoveredPortInfo && hoveredPortInfo.nodeId && hoveredPortInfo.portId) {
        ...
        if (portDetails) {
          ...
          // This case should ideally not happen if hoveredPortInfo is valid. Resetting.
        ...
        if (sourceNode && sourcePort) {
          ...
          // Do NOT call onConnectingEnd here. Keep connectingState active.
          // It will be reset by NodeCreationContextMenu's onClose or onSelect.
        } else {
            // Source port/node became invalid, reset connecting state.
            onConnectingEnd(null, null, 'input', {x:0,y:0});
        }
      }
      setHoveredPortInfo(null);
  ...
  const handleConnectionDoubleClick = (event: React.MouseEvent, connectionId: string) => {
    ...
    const updatedConnections = connections.map(conn => {
      if (conn.id === connectionId) {
        ...
        ].filter(p => p); // Ensure no null points if getPortPositionAndType fails (should not)

        if (pointsForSegments.length < 2) { // Should not happen for a valid connection
             newReroutePoints.push(worldPos); // Fallback: add to end
        } else {
            for (let i = 0; i < pointsForSegments.length - 1; i++) {
                const p1 = pointsForSegments[i];
                const p2 = pointsForSegments[i+1];
                
                // Simple distance to segment midpoint for now
            ...
            // Insert point into the reroutePoints array, not into pointsForSegments
            // If closestSegmentIndex is 0, it means insert after the source port, so at index 0 of reroutePoints
            // If closestSegmentIndex is pointsForSegments.length - 2, it means insert before the target port, so at the end of reroutePoints
        ...
        {connections.map((conn) => {
          ...
          const ambientPulsePath = !(isPulsingExecution || isPulsingData) ? (
            <path
              ...
              markerEnd={isExecutionConnection ? "url(#arrowhead-exec)" : "url(#arrowhead-data)"} // Use standard marker for ambient
            />
          ) : null;
        ...
        {nodes.map((node) => {
          ...
          return (
            <NodeComponent
              ...
              allNodesForConnectionContext={nodes}
              breakpoints={breakpoints}
              ...
              // Quick Inspect Props
              isQuickInspectTargetNode={isQuickInspectTargetNode} // Pass the function directly
...
// Helper function to determine if a node is in an error state
